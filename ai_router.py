...
class AIRouter:
    ...
    def _openai(self, item: ProviderKey, instructions: str, messages: list[dict[str, str]]) -> str:
        ...
        try:
            ...
            details = self._body_error(exc)
            if exc.code in {401, 403}: raise AIProviderError("authentication", f"OpenAI HTTP {exc.code}: {details}") from exc
            if exc.code == 429: raise AIProviderError("rate_limit", f"OpenAI HTTP 429: {details}") from exc
            if exc.code in {400, 404}: raise AIProviderError("model_error", f"OpenAI HTTP {exc.code}: {details}") from exc
        ...
        if not text: raise AIProviderError("provider_error", "OpenAI returned no text")
        return text

    def _gemini(self, item: ProviderKey, instructions: str, messages: list[dict[str, str]]) -> str:
        self._ensure_model(item)
        contents = [{"role": "user", "parts": [{"text": f"System instructions for Emily:\n{instructions}"}]}]
        ...
        try:
            ...
            details = self._body_error(exc)
            if exc.code in {401, 403}: raise AIProviderError("authentication", f"Gemini HTTP {exc.code}: {details}") from exc
            if exc.code == 429: raise AIProviderError("rate_limit", f"Gemini HTTP 429: {details}") from exc
            if exc.code in {400, 404}: raise AIProviderError("model_error", f"Gemini HTTP {exc.code}: {details}") from exc
        ...
        text = "\n".join(parts).strip()
        if not text: raise AIProviderError("provider_error", "Gemini returned no text")
        return text
    ...
    def generate_sync(self, instructions: str, messages: list[dict[str, str]]) -> str:
        ...
        for _ in range(len(self.keys)):
            ...
            try:
                ...
                print(f"{item.label} failed: {exc.error_type}")
    ...
    def status_text(self) -> str:
        ...
        with self._lock:
            ...
            for item in self.keys:
                if item.disabled: state = "disabled"
                elif item.cooldown_until > now: state = f"cooldown {max(1, int(item.cooldown_until - now))}s"
                else: state = "ready"
