...
# ============================================================
# Emily v2 — lightweight, SQLite-first Telegram AI companion
# ============================================================
VERSION = "2.0.0"
...
RUDE_KEYWORDS = {
    "stupid", "idiot", "dumb", "shut up", "hate you", "f**k", "fuck you"
}
...
def required_env(name: str) -> str:
    ...
    if not value:
        raise RuntimeError(f"{name} is required. Put it in your Termux environment or .env file.")
    return value
...
def migrate_legacy_database() -> None:
    ...
    try:
        with closing(sqlite3.connect(legacy)) as old, closing(db()) as new:
            ...
            old_users = old.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='users'").fetchone()
            if not old_users:
                return
            ...
            rows = old.execute(
                "SELECT user_id, username, daily_responses, total_responses, credits, "
                "last_reset_date, is_banned, roasts_count, last_interaction FROM users"
            ).fetchall()
            for row in rows:
                new.execute(
                    """
            ...
            new.commit()
            logger.info("Migrated %s users from legacy database", len(rows))
    except Exception:
...
# Quotas / credits
# Free = 50 replies/day
# Credit = exactly 1 AI generation
# Premium = higher daily quota, future special features
# -----------------------------
...
async def ask_ai(
    ...
    system = (
        ...
        f"Current personality mode: {mode_instruction} "
        f"Purpose: {purpose}. "
        "Use memory only when relevant; do not mention memory systems unless asked. "
    ...
    response = await ai.responses.create(model=MODEL, input=messages)
...
def log_error(error_type: str, operation: str, user_id: Optional[int], chat_id: Optional[int], details: str) -> None:
    ...
    with closing(db()) as conn:
        conn.execute(
            ...
            (error_type, operation, user_id, chat_id, details, now_iso()),
        )
        conn.commit()
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        ...
        "I'm a group-friendly AI companion. Mention me or reply to me in a group and I'll jump in.\n\n"
        "Try /mode, /memory, /remember, /quota, /moment and /help."
    )
    await update.effective_message.reply_text(text, reply_markup=mode_keyboard())
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.effective_message.reply_text(
        ...
        "/privacy — see memory/privacy controls\n"
        "/about — show version\n\n"
...
async def about(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.effective_message.reply_text(
        ...
        "Free: 50 AI replies/day\nCredits: 1 credit = 1 AI generation\nPremium: reserved for future higher quota + special features."
    )
...
async def forget_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if delete_memory(user.id, key):
        await message.reply_text(f"Forgot '{key}'. 🧹")
    else:
        await message.reply_text("I couldn't find that memory.")
...
async def privacy_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.effective_message.reply_text(
        "🔐 Emily privacy\n\n"
        "• Long-term memories are stored only in Emily's SQLite database.\n"
        "• You can inspect them with /memory.\n"
        "• Delete one with /forget key or everything with /forget_all.\n"
        "• Emily only uses a small recent conversation window plus relevant saved memories."
    )


# -----------------------------
# Emily Moments + group intelligence
# -----------------------------
async def moment_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        prompt = await ask_ai(
            ai, user.id, chat.id,
            "Create one short, creative, wholesome 'Emily Moment' for a Telegram user/group. "
            "It should invite a fun answer or tiny challenge. Avoid romance dependency. "
            "Return only the moment text with a fitting emoji.",
            get_user(user.id)["mode"],
            purpose="fun community moment",
        )
...
async def group_summary_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        summary = await ask_ai(
            ai, user.id, chat.id,
            "Summarize the following group-visible conversation in 5 concise bullets. "
            "Mention decisions, questions, useful facts and unresolved items. Don't invent anything.\n\n" + context_text,
            get_user(user.id)["mode"],
...
async def set_ban_state(update: Update, context: ContextTypes.DEFAULT_TYPE, state: int) -> None:
    if not update.effective_user or not is_admin(update.effective_user.id):
        return
    if not context.args:
        await update.effective_message.reply_text("Use the command with a numeric user ID.")
        return
    try:
    ...
    with closing(db()) as conn:
        ...
        conn.commit()
    await update.effective_message.reply_text(
        ("🚫 User banned." if state else "✅ User unbanned.") if result.rowcount else "User not found."
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Lightweight automatic memory extraction — deterministic, no second AI call.
    ...
    if not kind:
        await message.reply_text(
            f"I'm out of AI replies for now 🥺\n\n{quota_text(user.id)}\n\n"
    ...
    mode = row["mode"]
    purpose = "playful roast" if mode == "roast" or any(k in message.text.lower() for k in RUDE_KEYWORDS) else "normal chat"
    ai = context.application.bot_data["ai"]

    try:
        await context.bot.send_chat_action(chat_id=chat.id, action=ChatAction.TYPING)
        reply = await ask_ai(ai, user.id, chat.id, message.text, mode, purpose)
